...
#else
    log_goke( DEBUG_WARNING, "exec_t func not available in DEBUG version!");
#endif
    //clear fb cache
...
static int sd_mount() {
    ...
    if( ret != 0) {
        ...
        ret = system(cmd);
#else
        log_goke( DEBUG_WARNING, " sd_mount not available in DEBUG version!");
#endif
    }
    return ret;
}
...
#else
                log_goke( DEBUG_WARNING, " umount not available in DEBUG version!");
#endif
            }
            usleep(20*1000);
            memset(cmd, 0, sizeof(cmd));
