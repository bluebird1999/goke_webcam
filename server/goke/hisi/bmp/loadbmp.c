                  ...
                  , OSD_BITMAPINFO* pBmpInfo)
{
    ...
    {
        printf("Open file faild:%s!", filename);
        return -1;
    }
...
int LoadBMP(const char* filename, OSD_LOGO_T* pVideoLogo)
{
    ...
    {
        printf("fread error!line:%d", __LINE__);
        perror("fread:");
    }
...
int LoadBMPEx(const char* filename, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    if (fread(pOrigBMPBuf, 1, h * stride, pFile) != (h * stride) )
    {
        printf("fread (%d*%d)error!line:%d", h, stride, __LINE__);
        perror("fread:");
    }
        ...
        for (j = 0; j < w; j++)
        {
            if (Bpp == 3)
            {
                switch (enFmt)
                {
                    ...
                    default:
                        printf("file(%s), line(%d), no such format!", __FILE__, __LINE__);
                        break;
                }
            }
            else if ((Bpp == 2) || (Bpp == 4)) /*..............*/
...
int LoadBMPCanvas(const char* filename, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    ...
    {
        printf("Bitmap's stride(%d) is bigger than canvas's stide(%d). Load bitmap error!", stride, pVideoLogo->stride);
        free(pOrigBMPBuf);
        fclose(pFile);
        return -1;
    }
    ...
    {
        printf("Bitmap's height(%d) is bigger than canvas's height(%d). Load bitmap error!", h, pVideoLogo->height);
        free(pOrigBMPBuf);
        fclose(pFile);
        return -1;
    }
    ...
    {
        printf("Bitmap's width(%d) is bigger than canvas's width(%d). Load bitmap error!", w, pVideoLogo->width);
        free(pOrigBMPBuf);
        fclose(pFile);
        return -1;
    }
