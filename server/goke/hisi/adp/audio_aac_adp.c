...
static HI_S32 AACInitSbrEncLib(void)
{
    ...
    HI_VOID* pSbrEncHandle = AAC_SBRENC_GetHandle();
...
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_VOID* pSbrDecHandle = AAC_SBRDEC_GetHandle();
...
static HI_S32 AACInitEncLib(void)
{
    ...
    g_stAacEncFunc.pHI_AACENC_GetVersion = AACENC_GetVersion;
...
#ifdef HI_AAC_USE_STATIC_MODULE_REGISTER
    g_stAacDecFunc.pHI_AACDEC_GetVersion = AACDEC_GetVersion;
...
static HI_S32 AencCheckAACLCConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(pconfig->coderFormat == AACLC)
    {
        ...
        {
            printf("AACLC nChannelsOut(%d) in not equal to nChannelsIn(%d)", pconfig->nChannelsOut, pconfig->nChannelsIn);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        ...
        if(pconfig->sampleRate == 32000)
        {
            ...
            {
                printf("AACLC 32000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 44100)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 44100 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 48000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 48000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 16000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 16000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 8000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 8000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 24000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 24000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 22050)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLC 22050 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else
        {
            printf("AACLC invalid samplerate(%d)",pconfig->sampleRate);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
    }
    else
...
static HI_S32 AencCheckEAACConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(pconfig->coderFormat == EAAC)
    {
        if(pconfig->nChannelsOut != pconfig->nChannelsIn)
        {
            printf("EAAC nChannelsOut(%d) is not equal to nChannelsIn(%d)", pconfig->nChannelsOut, pconfig->nChannelsIn);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        ...
        if(pconfig->sampleRate == 32000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 32000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 44100)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 44100 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 48000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 48000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 16000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 16000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 22050)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 22050 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 24000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAAC 24000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else
        {
            printf("EAAC invalid samplerate(%d)",pconfig->sampleRate);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
    }
    else
...
static HI_S32 AencCheckEAACPLUSConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(pconfig->coderFormat == EAACPLUS)
    {
        ...
        {
            printf("EAACPLUS nChannelsOut(%d) and nChannelsIn(%d) should be 2",pconfig->nChannelsOut, pconfig->nChannelsIn);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        ...
        if(pconfig->sampleRate == 32000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 32000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 44100)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 44100 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 48000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 48000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 16000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 16000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 22050)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 22050 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 24000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("EAACPLUS 24000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else
        {
            printf("EAACPLUS invalid samplerate(%d)",pconfig->sampleRate);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
...
static HI_S32 AencCheckAACLDConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(pconfig->coderFormat == AACLD)
    {
        if(pconfig->nChannelsOut != pconfig->nChannelsIn)
        {
            printf("AACLD nChannelsOut(%d) in not equal to nChannelsIn(%d)", pconfig->nChannelsOut, pconfig->nChannelsIn);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        ...
        if(pconfig->sampleRate == 32000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 32000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 44100)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 44100 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 48000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 48000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 16000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 16000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 8000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 8000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 24000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 24000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 22050)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACLD 22050 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else
        {
            printf("AACLD invalid samplerate(%d)",pconfig->sampleRate);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
...
static HI_S32 AencCheckAACELDConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(pconfig->coderFormat == AACELD)
    {
        if(pconfig->nChannelsOut != pconfig->nChannelsIn)
        {
            printf("AACELD nChannelsOut(%d) in not equal to nChannelsIn(%d)", pconfig->nChannelsOut, pconfig->nChannelsIn);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
        ...
        if(pconfig->sampleRate == 32000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 32000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 44100)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 44100 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 48000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 48000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 16000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 16000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 8000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 8000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 24000)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 24000 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else if(pconfig->sampleRate == 22050)
        {
            ...
            if(pconfig->bitRate < s32MinBitRate ||  pconfig->bitRate > s32MaxBitRate)
            {
                printf("AACELD 22050 Hz bitRate(%d) should be %d ~ %d, recommed %d",pconfig->bitRate,s32MinBitRate,s32MaxBitRate,s32RecommemdRate);
                return HI_ERR_AENC_ILLEGAL_PARAM;
            }
        }
        else
        {
            printf("AACELD invalid samplerate(%d)",pconfig->sampleRate);
            return HI_ERR_AENC_ILLEGAL_PARAM;
        }
...
HI_S32 AencAACCheckConfig(AACENC_CONFIG *pconfig)
{
    ...
    if(NULL == pconfig)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "pconfig is null");
    ...
    {
        printf("aacenc coderFormat(%d) invalid",pconfig->coderFormat);
        return HI_ERR_AENC_ILLEGAL_PARAM;
    }
    ...
    {
        printf("aacenc quality(%d) invalid",pconfig->quality);
        return HI_ERR_AENC_ILLEGAL_PARAM;
    }

    if(pconfig->bitsPerSample != 16)
    {
        printf("aacenc bitsPerSample(%d) should be 16",pconfig->bitsPerSample);
        return HI_ERR_AENC_ILLEGAL_PARAM;
    }
    ...
    {
        printf("invalid transtype(%d), not in [0, 2]", pconfig->transtype);
        return HI_ERR_AENC_ILLEGAL_PARAM;
    }

    if(pconfig->bandWidth != 0 && (pconfig->bandWidth < 1000 || pconfig->bandWidth > pconfig->sampleRate / 2))
    {
        printf("AAC bandWidth(%d) should be 0, or 1000 ~ %d",pconfig->bandWidth,pconfig->sampleRate / 2);
        return HI_ERR_AENC_ILLEGAL_PARAM;
...
HI_S32 OpenAACEncoder(HI_VOID *pEncoderAttr, HI_VOID **ppEncoder)
{
    ...
    if (s32Ret)
    {
        printf("[Func]:%s [Line]:%d s32Ret:0x%x.#########", __FUNCTION__, __LINE__, s32Ret);
        return s32Ret;
    }
    ...
    if(NULL == pstEncoder)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "no memory");
    ...
    *ppEncoder = (HI_VOID *)pstEncoder;
    ...
    //printf("~~~~~~~~AENC AAC-TYPE:%d, trans-type:%d, bindWidth:%d ~~~~~~~~~",config.coderFormat,config.transtype,config.bandWidth);
                    ...
                    HI_U8 *pu8Outbuf,HI_U32 *pu32OutLen)
{
    ...
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "invalid AAC coder type");
        return HI_ERR_AENC_ILLEGAL_PARAM;;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "AAC encode failed");
    }

#ifdef DUMP_AACENC
    //printf("~~~~~~~~~strlen:%d~~~~~~", *((HI_S32*)pu32OutLen));
    //if (cnt_aenc > 0)
    {
    HI_U8 *pu8TestNum = pu8Outbuf + 1;
    //printf("Enc:(%02x %02x), ~~~~~~~~~strlen:%d~~~~~~", *pu8Outbuf, *pu8TestNum, *((HI_S32*)pu32OutLen));
...
HI_S32 OpenAACDecoder(HI_VOID *pDecoderAttr, HI_VOID **ppDecoder)
{
    ...
    *ppDecoder = (HI_VOID *)pstDecoder;
    ...
    /* create decoder */
    //printf("~~~~~~~~ADEC trans type:%d ~~~~~~~~~", pstAttr->enTransType);
    pstDecoder->pstAACState = AACInitDecoder_Adp((AACDECTransportType)pstAttr->enTransType);
    if (!pstDecoder->pstAACState)
    {
        free(pstDecoder);
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "AACInitDecoder failed");
                        ...
                        HI_U16 *pu16Outbuf,HI_U32 *pu32OutLen,HI_U32 *pu32Chns)
{
    ...
    *pu32Chns = 1;/*voice encoder only one channle */
    ...
    if (s32FrmLen < 0)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "AAC decoder can't find sync header");
    ...
    if (s32Ret)
    {
        printf("aac decoder failed!, s32Ret:0x%x", s32Ret);
        return s32Ret;
    }
    #if 0
    if (s32Ret)
    ...
    *pu32OutLen = s32SampleBytes;
    ...
    //if (cnt_adec > 0)
    {
        //printf("Dec: ~~~~~~~~(%d, %d)~~~~~~~", aacFrameInfo.outputSamps, *pu32OutLen);
...
HI_S32 ResetAACDecoder(HI_VOID *pDecoder)
{
    ...
    if (!pstDecoder->pstAACState)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s", __FUNCTION__, __LINE__, "AACResetDecoder failed");
        return HI_ERR_ADEC_DECODER_ERR;
    }
