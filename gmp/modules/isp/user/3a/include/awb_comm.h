...
typedef struct AWB_CCM_TAB_S {
    GK_U16 u16ColorTemp;                   /* RW;  Range:[2000,10000]; Format:16.0; the current color temperature */
...
typedef struct AWB_AGC_TABLE_S {
    ...
    GK_U8   au8Saturation[ISP_AUTO_ISO_STRENGTH_NUM];   /* RW;adjust saturation, different iso with different saturation */
} AWB_AGC_TABLE_S;

typedef struct AWB_SENSOR_DEFAULT_S {
    GK_U16  u16WbRefTemp;       /* RW;reference color temperature for WB  */
    GK_U16  au16GainOffset[ISP_BAYER_CHN_NUM];  /* RW; gain offset for white balance */
    GK_S32  as32WbPara[AWB_CURVE_PARA_NUM];      /* RW; parameter for wb curve,p1,p2,q1,a1,b1,c1 */

    GK_U16  u16GoldenRgain;      /* Rgain for the golden sample */
    GK_U16  u16GoldenBgain;      /* Bgain for the golden sample */
    GK_U16  u16SampleRgain;      /* Rgain for the current sample */
    GK_U16  u16SampleBgain;      /* Bgain for the current sample */
    AWB_AGC_TABLE_S stAgcTbl;
    AWB_CCM_S stCcm;
    GK_U16    u16InitRgain;           /* Init WB gain */
    GK_U16    u16InitGgain;
    GK_U16    u16InitBgain;
    GK_U8     u8AWBRunInterval;       /* RW;AWB Run Interval */
...
typedef struct AWB_SENSOR_EXP_FUNC_S {
    GK_S32 (*pfn_cmos_get_awb_default)(VI_PIPE ViPipe, AWB_SENSOR_DEFAULT_S *pstAwbSnsDft);
    GK_S32 (*pfn_cmos_get_awb_spec_default)(VI_PIPE ViPipe, AWB_SPEC_SENSOR_DEFAULT_S *pstAwbSpecSnsDft);
