 ...
 * 5. 事件也会进行拷贝，不受限制。事件和图片随后加入任务队列中
 * 6. 任务队列遵循FIFO进行处理，并发处理量为image_parallel
 * 7. 任务处理时，先上传事件。若事件上传失败，不会进行重传
 * 8. 上传图片。如果有图片数据，则进行图片上传，否则不做操作。如果图片上传失败，不会进行重传
 *
 *
 * @return lv_error_e
...
/*
 * @brief 部分SDK功能在编译阶段不存在问题，但在运行阶段会出现问题。
 *      SDK要触发此类功能，一般需要云端配合，调试复杂。
 *      该接口可在开发阶段即触发功能调试
 *
 * @notice 调用该接口，未出现LV-ERROR/LV_WARN级别日志即可
 *
 * @param [IN] type: 功能类型,见enum lv_develop_test_type_e.
 * @param [IN] ...: 可变长参数，参数类型与功能类型有关，具体参考sample
 *              type:LV_DEVELOP_TEST_PING, 参数：unsigned int times ,const char *address;
 *
 * @notice 未避免此函数被滥用，SDK会限制调用次数为3次，重新初始化SDK即可重置次数
 *
 *
 * @return lv_error_e
 */
int lv_develop_test(lv_develop_test_type_e type, ...);
